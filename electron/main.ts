...
ipcMain.handle('add-firmware-source', async (_event, firmware: FirmwareSource) => {
  return addFirmwareSource(firmware);
});

ipcMain.handle('remove-firmware-source', async (_event, firmwareId: string) => {
  return removeFirmwareSource(firmwareId);
});

ipcMain.handle('get-firmware-releases', async () => {
  const axios = await import('axios');
  const response = await axios.default.get('https://api.github.com/repos/THZoria/NX_Firmware/releases', {
