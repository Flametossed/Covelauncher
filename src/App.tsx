...
const FirmwarePage = ({ downloadDir, setDownloads }: {
  ...
  const handleDownloadProdKeys = async (url: string, name: string, referer: string) => {
    const id = Date.now().toString() + Math.random().toString(36).substring(7);
    ...
    try {
      await window.electronAPI.startDownload(id, url, referer || url, keysDir, true, name, undefined);
    } catch (err: any) {
      if (err.message === 'Download cancelled') return;
      setDownloads(prev => ({ ...prev, [id]: { ...prev[id], status: 'error', error: err.message } }));
    }
    ...
    <div className="flex-1 overflow-auto p-8 max-w-2xl mx-auto w-full flex flex-col gap-6">
      ...
      {loadingReleases ? (
        ...
        <div className="flex flex-col items-center justify-center gap-4 text-slate-500 py-24">
          ...
          <p className="text-lg font-medium">No firmware source configured</p>
          <p className="text-sm text-slate-500">Add the Games-FW-Keys source in Settings to access firmware and keys.</p>
        </div>
      ) : (
        <>
          ...
          <div className="bg-slate-800/60 border border-slate-700/60 rounded-2xl overflow-hidden">
            ...
            <div className="p-6 flex flex-col gap-4">
              ...
              <p className="text-xs text-slate-500">
                {releases.length} firmware version{releases.length !== 1 ? 's' : ''} available
              </p>
            </div>
          </div>
        </>
      ...
      <div className="bg-slate-800/60 border border-slate-700/60 rounded-2xl overflow-hidden">
        ...
        <div className="p-6">
          {loadingProdKeys ? (
            ...
            <div className="flex gap-3">
              <button
                onClick={() => handleDownloadProdKeys(prodKeys.downloadUrl, `prod.keys ${prodKeys.version}`, prodKeys.referer)}
            ...
            <p className="text-sm text-slate-500">No download link found</p>
          )}
        </div>
      </div>
    </div>
...
function App() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setLatestGames(latest.filter(g => g.imageUrl));
      setPopularGames(popular.filter(g => g.imageUrl));
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const filtered = results.filter(g => g.imageUrl);
      setGames(filtered);
      if (filtered.length === 0) setError('No games found.');
    } catch (err: any) {
